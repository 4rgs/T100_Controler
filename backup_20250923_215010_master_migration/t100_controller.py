...
class T100Controller:
    ...
    def __init__(self):
        ...
        print("🎮 T100 ZK-5AD + MG90S Controller inicializado")
        ...
        while self.is_running:
            ...
            try:
                ...
                if channels:
                    ...
                    raw_forward_backward = channels.get('CH2', 992)  # CH2: Aceleración (adelante/atrás)
                    raw_left_right = channels.get('CH1', 992)        # CH1: Rotación (izquierda/derecha)
                    
                    # Extraer controles de cámara (valores raw del ELRS)
                    raw_pan = channels.get('CH4', 992)   # CH4: Pan cámara (izquierda/derecha)
                    raw_tilt = channels.get('CH3', 992)  # CH3: Tilt cámara (arriba/abajo)
                    
                    # Convertir valores raw ELRS a rango -1.0 a 1.0 para el driver de motores
                    ...
                    # Aplicar control de servos de cámara
                    ...
                    if self.loop_count % 100 == 0:
                        self.logger.info(
                            f"TANK CH1:{raw_left_right:4d} CH2:{raw_forward_backward:4d} LR:{left_right:+.2f} FB:{forward_backward:+.2f} PWM A:{pwm_a:3d} B:{pwm_b:3d} | "
                            f"CAM CH3:{raw_tilt:4d} CH4:{raw_pan:4d} Tilt:{tilt_degrees:5.1f}° Pan:{pan_degrees:5.1f}°"
                        )
                    ...
                    if time_since_last_command > self.failsafe_timeout:
                        ...
                        self.servo_driver.center_servos()  # Centrar servos en failsafe
                        if self.loop_count % 100 == 0:
                            self.logger.warning("⚠️  FAILSAFE: Sin señal ELRS - Motores parados, servos centrados")
    ...
    async def cleanup(self):
        """Limpia recursos, para motores y centra servos."""
...
async def main():
    """Función principal."""
    print("🚀 T100 ZK-5AD + MG90S Controller")
    print("=" * 55)
    print("🎮 CONTROLES:")
    print("   Joystick DERECHO:")
    print("     CH2 (Vertical)   → Tank Forward/Backward")
    print("     CH4 (Horizontal) → Tank Left/Right")
    print("   Joystick IZQUIERDO:")  
    print("     CH1 (Horizontal) → Cámara Pan (0-180°)")
    print("     CH3 (Vertical)   → Cámara Tilt (0-180°)")
    print("🛑 Parar: Ctrl+C")
    print("=" * 55)
